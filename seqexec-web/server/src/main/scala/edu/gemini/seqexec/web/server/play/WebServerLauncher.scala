...
import edu.gemini.seqexec.web.common.{Sequence, SequenceSteps, Step, StepConfig}
      ...
      lazy val router = Router.from {
        ...
        case GET(p"/api/sequence/$id<.*-[0-9]+>") => Action {
          ...
          ExecutorImpl.read(obsId) match {
            ...
            case -\/(e) => {println(SeqexecFailure.explain(e));Results.NotFound(SeqexecFailure.explain(e))}
          }
        }
        case GET(p"/$f*") =>
          // Static files,
